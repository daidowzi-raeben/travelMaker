  ...
  <div>
    ...
    <div class="wrap" style="display: none">
      <div class="content">
        ...
        <div class="list-option">
          ...
          <div class="right">
            <button
              ...
              @click="onClickListMode('list')"
            ></button>
            <button
              type="button"
              class="btn btn-normal"
              :class="{ active: listMode == 'normal' }"
              @click="onClickListMode('normal')"
            ></button>
            <button
              type="button"
              class="btn btn-image"
              :class="{ active: listMode == 'image' }"
              @click="onClickListMode('image')"
            ></button>
          </div>
        </div>
        <div>
          <Adsense
            data-ad-client="ca-pub-8363464671585235"
            data-ad-slot="9977730666"
            data-ad-format="auto"
        ...
        <ul
          v-if="EVENT_DATA.LIST != ''"
          class="thumb-list"
          :class="{ list: listMode == 'list', image: listMode == 'image' }"
        >
          <template v-for="(v, i) in EVENT_DATA.LIST">
            <li
              v-if="i < isRowItemCnt * isPage"
              :key="i"
              class="thumb-list--item"
              @click="onClickToDetail(v)"
            >
              <div class="thumb">
      ...
      <div class="modal" v-bind:class="{ show: filterModalShow }">
        ...
        <div class="modal-con bottom">
          ...
          <div class="input-wrap">
            ...
            <el-date-picker
              v-model="filterData.edDt"
              :picker-options="onLoadDisabledDate(filterData.stDt)"
              type="date"
              placeholder="날짜선택"
            >
            </el-date-picker>
          ...
          <div class="input-wrap">
            ...
            >
              <el-option
                v-for="(v, i) in LOCATION_CODE[`gu${filterData.do}`]"
                :key="i"
                :label="v.guName"
                :value="v.guCode"
              >
        ...
        <div class="input-wrap">
          ...
          <div class="btn-wrap">
            <el-button type="primary" @click="onClickFilter"
              >필터적용</el-button
            >
          </div>
        </div>
      </div>
      <router-link to="/map" class="bottom-btn"
        ><span class="icon icon-map"></span>지도보기</router-link
      >
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    if (localStorage.getItem('listMode'))
      this.onClickListMode(localStorage.getItem('listMode'))
  ...
  methods: {
    ...
    onLoadGuName(v) {
      for (
        let i = 0;
        i < this.LOCATION_CODE[`gu${this.filterData.do}`].length;
        i++
      ) {
